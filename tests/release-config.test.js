...
test('all review targets use the approved bundle namespace and version', () => {
  ...
  assert.match(project, /PRODUCT_BUNDLE_IDENTIFIER = uk\.acutemedicine\.acutemedicaltake\.watch\.extension;/);
  assert.equal((project.match(/CURRENT_PROJECT_VERSION = 2;/g) || []).length, 8);
  assert.equal((project.match(/MARKETING_VERSION = 1\.0\.0;/g) || []).length, 8);
});
